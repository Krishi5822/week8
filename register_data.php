...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $cmd -> bindParam(':username', $email, PDO::PARAM_STR, 40);
...
<body>
    <div class="container">
        ...
        <h1 class="card-title fs-5 mt-4 text-center">CREATE PROFILE</h1>
        ...
        <form  novalidate class="p-5" method="POST">
            ...
            <div class="form-floating mb-4">
            <input type="password" required name="new-password" class=" <?= (isset($errors['password']) ? 'is-invalid' : '') ?> rounded-0 form-control" id="new-password" placeholder="new-password"value="<?= $new_password ?? '' ?>" >
            <label for="new-password"> New Password </label>
            <p class="text-danger"> <?= $errors['password'] ?? ''; ?> </p>
            </div>

            <div class="form-floating mb-4">
            <input type="password" required name="confirm-password" class="<?= (isset($errors['confirm']) ? 'is-invalid' : '') ?> rounded-0 form-control" id="confirm-password" 
        ...
        <p class="text-center"> Already have an account? <a href="login.php" class="text-warning"> <strong> Login here </strong></p>
        </div>
        </div>
    </div>
